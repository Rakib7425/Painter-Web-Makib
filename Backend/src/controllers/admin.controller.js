...
const getAdminChannelProfile = asyncHandler(async (req, res) => {
	...
	const channel = await Admin.aggregate([
		...
		},
		{
			$addFields: {
				...
				isSubscribed: {
					$cond: {
						if: { $in: [req.admin?._id, "$subscribers.subscriber"] },
						then: true,
						else: false,
					},
				},
	...
	return res
		.status(200)
		.json(new ApiResponse(200, channel[0], "Admin channel fetched successfully"));
});
